...
export function resolveModelConfig(
  ...
  // 如果四级降级后仍拿不到 apiKey，尝试从供应商列表自动匹配
  if (!apiKey) {
    const providers = getProviders()
    const hasValidProvider = providers.some(p => p.apiKey && p.baseURL && p.models?.length)
    if (hasValidProvider) {
      model = undefined
      channel = undefined
      for (const p of providers) {
...
export function resolveProviderConfig(providerKey: string): { baseURL: string; apiKey: string } | null {
  ...
  let userProvider = userProviders.find((p) => p.key === providerKey || p.id === providerKey)

  // 兜底：旧数据可能存的是name，按name再匹配一次
  if (!userProvider) {
    userProvider = userProviders.find((p) => p.name === providerKey)
  }
