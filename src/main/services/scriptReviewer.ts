...
import { callAI, loadPromptTemplate } from './ai'
...
async function deepCheck(script: string): Promise<ReviewFinding[]> {
  ...
  const systemPrompt = loadPromptTemplate('script_review_check', '你是抖音平台漫剧内容审核员。严格审查剧本，逐条覆盖六大红线(RL-01~RL-06)：不良价值观、低俗色情、封建迷信、观感不适、侵权肖像、危害未成年及公序良俗。每项规则必须审查，即使无问题也标注hasIssue:false。返回JSON{findings:[{ruleId,category,severity,ruleName,passed,details,matchedContent,suggestions}],overallVerdict,score,summary}。宁可多报不可漏报。')
...
返回JSON格式，findings必须包含全部16条规则：
{
  ...
  try {
    ...
    try {
      ...
      logger.error('[deepCheck] 无法解析AI返回的JSON:', aiResult.slice(0, 500))
    }

    // 检查AI返回是否有效
    if (!aiData || !aiData.findings || aiData.findings.length < 10) {
      logger.error(`[deepCheck] AI返回不完整(${aiData?.findings?.length || 0}条规则)，回退到快速检查`)
    ...
    logger.error('[deepCheck] AI审查失败，回退到快速检查:', err)
...
export async function autoFixScript(
  ...
  const systemPrompt = loadPromptTemplate('script_review_fix_all', '你是漫剧剧本修改专家。根据审查发现的所有问题，对剧本进行完整修改。修改原则：1.保持原有剧情结构和人物设定 2.删除违规台词和描写 3.低俗内容替换为正常表达 4.暴力描写弱化或省略 5.侵权元素替换为原创 6.保持角色性格和故事走向不变。返回JSON{modifiedScript,changes:[{original,suggestion,reason}]}。')
...
请以JSON格式返回修改结果：
{
  ...
  try {
    ...
    logger.error('[autoFixScript] AI修改失败:', err)
    throw err
  }
}
...
export async function generateFixSuggestion(
  ...
  const systemPrompt = loadPromptTemplate('script_review_fix_one', '你是漫剧剧本修改专家。针对剧本中一个具体问题生成修改方案。保持原剧本风格和人物性格不变，只修改违规部分。返回JSON{fixedSnippet,reason}。')
  const problemDetail = finding.details || finding.ruleName || ''
  const matchedText = finding.matchedContent?.join('\n') || ''
  const userPrompt = `剧本片段：\n${script}\n\n问题类型：${finding.ruleName}\n问题详情：${problemDetail}\n匹配内容：${matchedText}`
  ...
  try {
    ...
    logger.error('[generateFixSuggestion] 失败:', err)
