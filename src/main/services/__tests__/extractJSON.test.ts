...
describe('extractJSON', () => {
  ...
  it('纯文本被 jsonrepair 包装为 JSON 字符串', () => {
    const result = extractJSON('this is just plain text no json')
    expect(JSON.parse(result)).toBe('this is just plain text no json')
  })

  it('数组中含带引号的字符串值', () => {
    const input = '[{"dialogue":"她说了\\"你好\\"然后离开","index":1}]'
    const result = extractJSON(input)
    expect(JSON.parse(result)).toEqual([{ dialogue: '她说了"你好"然后离开', index: 1 }])
  })

  it('从含前缀后缀的文本中提取JSON数组', () => {
    const input = '解析结果如下：\n[{"name":"角色A","type":"主角"},{"name":"角色B","type":"配角"}]\n以上为全部角色。'
    const result = extractJSON(input)
    expect(JSON.parse(result)).toEqual([
      { name: '角色A', type: '主角' },
      { name: '角色B', type: '配角' }
