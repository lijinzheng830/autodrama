...
### 必须遵守
1. **只拆解视觉内容** — shot_description 中不包含对白、不写"他说"、不写内心想法。只描述眼睛能看到的东西。
2. **角色名称必须精确匹配** — 角色名必须来自可用角色列表。如果角色名未出现，该字段用空数组 []。
3. **场景必须精确匹配** — 场景名必须来自可用场景列表。如果场景不明确，用最贴近的场景名。
4. **景别 7 选 1，不可自造** — 大远景 / 远景 / 全景 / 中景 / 近景 / 特写 / 大特写
5. **运镜 8 选 1，不可自造** — 固定 / 缓慢推进 / 缓慢拉远 / 左摇 / 右摇 / 跟随 / 环绕 / 升降
