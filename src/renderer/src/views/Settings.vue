...
const providerForm = ref({ name: '', key: '', baseURL: '', apiKey: '', models: [] as Array<{ key: string; name: string; type: string }> })
...
function openEditProvider(p: ProviderItem): void {
  ...
  providerEditId.value = (p as any).id || p.key || ''
  providerForm.value = {
    name: p.name || '',
    key: p.key || '',
    baseURL: p.baseURL || '',
    apiKey: p.apiKey || '',
    models: Array.isArray(p.models) ? p.models.map((m: any) => typeof m === 'string' ? { key: m, name: m, type: 'text' } : { key: m.key || m.name || '', name: m.name || m.key || '', type: m.type || 'text' }) : []
  }
  providerDialogVisible.value = true
}
...
function addProviderModel(): void {
  providerForm.value.models.push({ key: '', name: '', type: 'text' })
}
...
async function saveProvider(): Promise<void> {
  ...
  // Key：手动指定优先，否则从baseURL自动提取
  const extractedKey = key?.trim() || extractKeyFromBaseURL(baseURL)
  ...
  const finalKey = generateUniqueKey(extractedKey, existingKeys)

  const modelList = models
    .filter((m) => m.name.trim())
    .map((m) => ({ key: (m as any).key?.trim() || m.name.trim(), name: m.name.trim(), type: m.type, free: false }))
  const data = { name, key: finalKey, baseURL, apiKey, models: modelList as any }
  try {
...
async function handleExportConfig(): Promise<void> {
  try {
    ...
    const filePath = await window.api.showSaveDialog({
      ...
      filters: [{ name: 'Endless Ideas配置', extensions: ['autodrama-config'] }]
    })
    if (!filePath) return
  ...
  <div class="settings-layout">
                ...
                <tr v-for="p in providers" :key="p.id">
                  ...
                  <td>
                    <el-button text size="small" :icon="Edit" @click="openEditProvider(p)"
                      >编辑</el-button
                    >
                    <el-button text size="small" :icon="Delete" @click="handleDeleteProvider((p as any).id || p.key || '')"
                      >删除</el-button
                    >
                  </td>
                </tr>
        ...
        <div v-if="activeNav === 'routes'" class="settings-panel">
          ...
          <div class="panel-body">
            <table class="data-table route-table">
              <thead>
                <tr><th>用途</th><th>模型</th><th>渠道</th></tr>
              </thead>
              <tbody>
                <tr v-for="rp in routePurposes" :key="rp.key">
                  <td>{{ rp.label }}</td>
                  <td>
                    <el-select :model-value="getRouteModel(rp.key)" size="small" class="dark-select" placeholder="选择模型" @update:model-value="(val: string) => setRouteModel(rp.key, val)">
                      <el-option label="未设置" value="" />
                      <el-option v-for="m in getRouteModelOptions(rp.key)" :key="m.value" :label="m.label" :value="m.value" />
                    </el-select>
                  </td>
                  <td>
                    <el-select :model-value="getRouteChannel(rp.key)" size="small" class="dark-select" placeholder="选择渠道" @update:model-value="(val: string) => setRouteChannel(rp.key, val)">
                      <el-option label="未设置" value="" />
        ...
        <div v-if="activeNav === 'about'" class="settings-panel">
          <div class="about-section">
            <div class="about-logo">
              <img src="/icon.png" alt="Endless Ideas" class="about-icon" />
              <h2 class="about-title">Endless Ideas</h2>
    ...
    >
      <div class="dialog-form">
        ...
        <div class="form-row">
          <label class="form-label">Key <span style="color:#6b7280;font-size:11px">（唯一标识，留空则自动从URL提取）</span></label>
