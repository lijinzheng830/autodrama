...
const api = {
  ...
  deleteParseGroup: (projectId: string, parseGroup: number) => ipcRenderer.invoke('project:deleteParseGroup', projectId, parseGroup),
  ...
  autoProcessSeedance: (projectId: string, script: string, options?: AutoProcessOptions) =>
    ipcRenderer.invoke('ai:auto-process-seedance', { projectId, script, options }),
  ...
  saveShotField: (shotId: string, field: string, value: unknown) =>
    ipcRenderer.invoke('shot:saveField', { shotId, field, value }),
  ...
  // Grid Storyboard
  generateStoryboard: (input: { projectId: string; shotIds?: string[]; model?: string; channel?: string; resolution?: string }) =>
    ipcRenderer.invoke('storyboard:generate', input),
