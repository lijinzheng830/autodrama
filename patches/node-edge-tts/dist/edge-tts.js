...
class EdgeTTS {
    constructor({ voice = 'zh-CN-XiaoyiNeural', lang = 'zh-CN', outputFormat = 'audio-24khz-48kbitrate-mono-mp3', saveSubtitles = false, proxy, rate = 'default', pitch = 'default', volume = 'default', timeout = 10000, style = 'neutral' } = {}) {
        this.voice = voice;
        this.lang = lang;
    ...
    async ttsPromise(text, audioPath) {
      ...
      ` + `<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="https://www.w3.org/2001/mstts" xml:lang="${this.lang}">
        <voice name="${this.voice}">
          ` + (this.style && this.style !== 'neutral' ? `<mstts:express-as style="${this.style}">` : ``) + `
            <prosody rate="${this.rate}" pitch="${this.pitch}" volume="${this.volume}">
              ${escapeXml(text)}
            </prosody>
          ` + (this.style && this.style !== 'neutral' ? `</mstts:express-as>` : ``) + `
        </voice>
      </speak>`);
        });
    }
